#!/usr/bin/env bash
# local-manage-packages.sh
# Interactive package and hook manager — non-submodule variant.
# Use when the project does NOT have software-dev-agentic as a git submodule.
# Files are COPIED (not symlinked). Run this script from the software-dev-agentic repo.
#
# Usage:
#   scripts/local-manage-packages.sh --platform=web --project=/path/to/project
#   scripts/local-manage-packages.sh --platform=ios --project=/path/to/project
...
pkg_is_enabled() {
  # A package is considered enabled if at least one of its files exists
  local pkg_file="$1"
  local agents skills
  agents="$(read_pkg "$pkg_file" agents)"
  skills="$(read_pkg "$pkg_file" skills)"
  for agent in $agents; do
    [ -f "$CLAUDE_DIR/agents/$agent.md" ] && return 0
  done
  for skill in $skills; do
    [ -d "$CLAUDE_DIR/skills/$skill" ] || [ -f "$CLAUDE_DIR/skills/$skill" ] && return 0
  done
  return 1
}
...
enable_pkg() {
  ...
  for agent in $agents; do
    ...
    if [ -e "$dest" ]; then
      echo "    skip  $agent (already present)"
    else
      cp -f "$src" "$dest"
      echo "    $(green "copy")  $agent"
    fi
  done
...
disable_pkg() {
  ...
  for agent in $agents; do
    local dest="$CLAUDE_DIR/agents/$agent.md"
    if [ -f "$dest" ] && [ ! -L "$dest" ]; then
      rm "$dest"
      echo "    $(red "remove")  $agent"
    fi
  done
  ...
  # Record in disabled list so enable_pkg can re-copy later
...
echo ""
echo "$(bold "software-dev-agentic package manager") (platform: $(bold "$PLATFORM") · project: $(dim "$PROJECT_ROOT"))"
echo "────────────────────────────────────────────────────────"
