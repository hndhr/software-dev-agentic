> Related: [aegis-kms-load skill](../../skills/procedures/aegis-kms-load/SKILL.md) · [kms-conventions.md](../../../../docs/principles/kms/kms-conventions.md)

Standard output format produced after every `aegis-kms-load` call. Agents emit this block; orchestrators and calling skills can parse it by heading.
...
- One `## Knowledge Loaded` block per `aegis-kms-load` call. Two calls → two blocks, each labelled with their own `{discipline}/{artifact}`.
