...
```bash
ls "$PROJECT_ROOT/.claude/agentic-state/developer/feature-plans/" 2>/dev/null || echo "(runs/ is already empty)"
```
...
```bash
ls "$PROJECT_ROOT/.claude/agentic-state/developer/feature-plans/" && echo "(done)" || echo "(done — runs/ is now empty)"
```
