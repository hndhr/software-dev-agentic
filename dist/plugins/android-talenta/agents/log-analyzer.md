...
name: log-analyzer
description: >
  Analyze Mekari Log entries to build a user activity timeline, identify errors,
  find root causes, and propose code fixes. Use when the user has raw log JSON
  (from Grafana Loki or the loki-log-query agent) and wants to understand what
  happened during a user session. Works with Flutter (mobile-talenta),
  Swift (talenta-ios), and Kotlin (talenta-mobile-android) codebases.
...
## Step 0: Ask for Project Path (if fix proposals are needed)

At the start of the session, ask:
> "Do you want me to propose code fixes for any errors found?
> If yes, please provide the path to the relevant project:
> - Flutter: /path/to/mobile-talenta
> - iOS: /path/to/talenta-ios
> - Android: /path/to/talenta-mobile-android"

Store this as `PROJECT_PATH`. If the user skips this, omit the fix proposals section.
