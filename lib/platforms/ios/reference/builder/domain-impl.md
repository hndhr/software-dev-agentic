...
> Concepts and invariants: `reference/builder/domain-theory.md`. This file covers Swift syntax and iOS-specific patterns.
...
Pure business decisions — no I/O, no side effects, no async. Can be called by both UseCases and ViewModels. See extraction rules in `reference/builder/domain-theory.md`.
...
`BaseErrorModel` is the canonical error type for all UseCase and Repository completions (`Result<Model, BaseErrorModel>`). Repositories map `NetworkError` → `BaseErrorModel` before propagating upward. See `reference/builder/error-handling-impl.md` for full error flow and mapping patterns.
