...
description: Investigate a Talenta Android TS (Technical Support) issue report. Classifies the issue, gathers Crashlytics and/or Loki evidence, traces the implicated code, and proposes ranked fixes. Supports ANR, Crash, API, and other issue classes.
user-invocable: true
...
- Contains `talenta-mobile-android` → repo confirmed as `talenta-mobile-android`
- Ambiguous → AskUserQuestion: "Which repo are we working in? (`mobile-talenta` / `talenta-mobile-android`)"
...
2. "Which Android OS version was the affected user on? (e.g. Android 14)"
