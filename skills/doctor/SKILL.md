---
name: doctor
description: Audit the web-agentic setup in a downstream project — checks submodule, symlinks, CLAUDE.md markers, settings, and GitHub CLI auth.
user-invocable: true
...
```bash
git submodule status .claude/web-agentic
```
...
```bash
git -C .claude/web-agentic fetch --quiet origin main 2>/dev/null
git -C .claude/web-agentic rev-list --count HEAD..origin/main
```
...
⚠  submodule    3 commits behind origin/main — run: .claude/web-agentic/scripts/sync.sh
✓  agents       9 linked
✗  skills       .claude/skills/ missing — run: .claude/web-agentic/scripts/setup-symlinks.sh
